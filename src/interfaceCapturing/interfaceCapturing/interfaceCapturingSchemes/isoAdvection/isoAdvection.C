...
:
    ...
    nAlphaBounds_(interfaceCapturing::modelDict().lookupOrDefault<label>("nAlphaBounds", 3)),
    isoFaceTol_(interfaceCapturing::modelDict().lookupOrDefault<scalar>("isoFaceTol", 1e-8)),
    surfCellTol_(interfaceCapturing::modelDict().lookupOrDefault<scalar>("surfCellTol", 1e-8)),
    writeIsoFacesToFile_(interfaceCapturing::modelDict().lookupOrDefault("writeIsoFaces", false)),
...
void Foam::advection::isoAdvection<reconstructionScheme>::applyBruteForceBounding()
{
    ...
    scalar snapAlphaTol = modelDict().lookupOrDefault("snapTol", 0);
    if (snapAlphaTol > 0)
    {
        alpha1_ =
            alpha1_
